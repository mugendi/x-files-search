...
<nav aria-label="Pagination">
	<ul class="pagination">
		...
		{#each paginationArray as page, i}
			<li class="page-item" class:active={page === currentPage}>
				{#if i == 1 && enterPageNum}
					<input class="page-link" type="number" value={page} min="1" max={totalPages} on:keydown={setCurrentPage} />
				{:else}
					<button
						class="page-link"
						...
						class:expanded={i == 1}
						on:dblclick={() => {
							if (i == 1) enterPageNum = !enterPageNum;
						}}
					>
						{page}
					</button>
